...
@runtime_checkable
class HealProvenanceStore(Protocol):
    async def put_heal_provenance(
        self, event_id: str, group: str, witness_pubkeys: Sequence[str], ts: int
    ) -> None: ...
    async def get_heal_provenance(self, event_id: str) -> list[str]: ...
    def iter_events_admitted_by(self, witness_pubkey: str, group: str | None = None) -> AsyncIterator[str]: ...
